...
namespace FFXINAVBUILDER.Common
{
    ...
    public class DAT
    {
        ...
        public DAT(MainForm mf, string fn)
        {
            Mainform = mf;
            FileName = fn;
        }
        ...
        public void Parse1C(CHUNK c)
        {
            ...
            using (var sw = new StreamWriter(string.Format(@"{0}\Map Collision obj files\{1}.obj", Application.StartupPath, FileName)))
            {
        ...
        private void ParseGridEntry(byte[] data, int entryoffs, int x, int y)
        {
            ...
            {
                Mainform.Logger.AddDebugText(Mainform.rtbDebug, ex.ToString());
            }
        }
